...
function resolveWithBaseDir(relPath) {
...
async function checkPermissions(fullPath) {
  try {
    ...
    const stat = await fs.stat(fullPath);
    const canWrite = await fs.access(fullPath, fs.constants.W_OK).then(() => true).catch(() => false);
...
app.get('/api/list/:path(*)', async (req, res) => {
  try {
  ...
  try {
    ...
    bb.on('close', async () => {
      ...
      try {
        ...
        const newRelPath = path.relative(BASE_DIR, finalPath);
        res.json({ ok: true, value: newRelPath });
      } catch (err) {
...
app.listen(PORT, () => {
  console.log(`NHFS server running at http://localhost:${PORT}`);
});
